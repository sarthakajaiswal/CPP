...
Organization::Organization(std::string name, std::string state, std::string country) : 
    m_name(name), m_address(state, country) {} 
...
std::ostream& operator<<(std::ostream& os, const Organization& obj) {
    os << obj.m_name << obj.m_address; 
        
    if(obj.m_email != "")
        os << ", email- " << obj.m_email; 
    if(obj.m_contacts.size() != 0)
        os << ", call- " << obj.m_contacts; 
